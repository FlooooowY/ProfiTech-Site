...
async function checkDatabase() {
  ...
  try {
    ...
    const [productsResult] = await connection.execute('SELECT COUNT(*) as count FROM products') as any[];
    const productCount = productsResult[0].count;
    console.log(`📦 Товаров в базе: ${productCount.toLocaleString()}`);

    // 2. Количество категорий
    const [categoriesResult] = await connection.execute('SELECT COUNT(*) as count FROM categories') as any[];
    const categoryCount = categoriesResult[0].count;
    console.log(`📁 Категорий: ${categoryCount}`);

    // 3. Количество подкатегорий
    const [subcategoriesResult] = await connection.execute('SELECT COUNT(*) as count FROM subcategories') as any[];
    const subcategoryCount = subcategoriesResult[0].count;
    console.log(`📂 Подкатегорий: ${subcategoryCount}`);

    // 4. Количество характеристик
    const [characteristicsResult] = await connection.execute('SELECT COUNT(*) as count FROM product_characteristics') as any[];
    ...
    const [imagesResult] = await connection.execute('SELECT COUNT(*) as count FROM product_images') as any[];
    ...
    manufacturersStats.forEach((stat: any, index: number) => {
